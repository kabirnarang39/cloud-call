        ...
        .then(stream => {
         ...
         console.log("stream.getTracks() ", stream.getTracks())
            for (let socket_id in peers) {
                ...
                for (let index in peers[socket_id].options._streams[0].getTracks()) {
                    for (let index2 in stream.getTracks()) {
                        if (peers[socket_id].options._streams[0].getTracks()[index].kind === _stream.getTracks()[index2].kind) {
                            peers[socket_id].replaceTrack(peers[socket_id].options._streams[0].getTracks()[index], _stream.getTracks()[index2], peers[socket_id].options._streams[0])
                            break;
                        }
                    }
                }
